...
namespace SpamClassificationSystem
{
    internal class Program
    {
        static void Main(string[] args)
        {
            try
            {
                ...
                else if (args.Length == 2)
                {
                    ...
                    string outputFileName =
                        $"{inputFileNameWithoutExtension}_output_{currentDate}.csv";
                    ...
                    Console.WriteLine();
                    Console.WriteLine(
                        $"Predictions were written to: {outputPath}");
                }
                else
                {
                    PrintUsage();
            ...
            {
                Console.WriteLine(
                    $"File not found: {Path.GetFileName(ex.FileName)}");
            }
            catch (InvalidDataException ex)
            {
                Console.WriteLine(
                    $"Invalid CSV data: {ex.Message}");
            }
